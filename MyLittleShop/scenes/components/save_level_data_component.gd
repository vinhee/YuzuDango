extends Node
class_name SaveLevelData

var level_scene_name: String
var save_game_datapath: String = "user://game_data"
...
func save_node_data() -> void:
	...
	if nodes != null:
		for node in nodes:
			if node is SaveData:
				var save_data_resource: DataResource = node.save_data()
				var save_final_resource = save_data_resource.duplicate()
				game_data_resource.save_data_nodes.append(save_final_resource)

func save_game() -> void:
	if !DirAccess.dir_exists_absolute(save_game_datapath):
		DirAccess.make_dir_absolute(save_game_datapath)
	
	var same_level_file_name = save_file_name % level_scene_name
...
func load_game() -> void:
	...
	var save_game_path: String = save_game_datapath + level_save_file_name
